    ...
    public class CreateCarWorkshopServiceCommandHandlerTests
    {
        ...
        public async Task Handle_CreatesCarWorkshopService_WhenUserIsAuthorized()
        {
            ...
            userContextMock.Setup(c => c.GetCurrentUser())
                .Returns( new CurrentUser("1", "user1", new[] { "User" }));

            var carWorkshopRepositoryMock = new Mock<ICarWorkshopRespository>();
            ...
            carWorkshopServiceRepositoryMock.Verify(r => r.Create(It.IsAny<Domain.Entities.CarWorkshopService>()), Times.Once);
        ...
        public async Task Handle_CreatesCarWorkshopService_WhenUserIsModerator()
        {
            ...
            var userContextMock = new Mock<IUserContext>();
            userContextMock.Setup(c => c.GetCurrentUser())
                .Returns(new CurrentUser("2", "user1", new[] { "Moderator" }));
        ...
        public async Task Handle_DoesntCreatesCarWorkshopService_WhenUserIsNotAuthorized()
        {
            ...
            var userContextMock = new Mock<IUserContext>();
            userContextMock.Setup(c => c.GetCurrentUser())
                .Returns(new CurrentUser("2", "user1", new[] { "User" }));
            ...
            //assert
            result.Should().Be(Unit.Value);
            carWorkshopServiceRepositoryMock.Verify(r => r.Create(It.IsAny<Domain.Entities.CarWorkshopService>()), Times.Never);
